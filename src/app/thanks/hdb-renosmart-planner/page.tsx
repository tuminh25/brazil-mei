// src/app/thanks/hdb-renosmart-planner/page.tsx
// Post-Purchase thank-you page for the HDB RenoSmart Budget & Compliance Planner.
...
import { HDB_RENOSMART_PRODUCT } from "@/config/products";
...
export const metadata: Metadata = {
  title: "Your HDB RenoSmart Planner Is Ready — SGEventsHub",
  robots: { index: false, follow: false },
};

export default function HDBRenoSmartPlannerThanksPage() {
  const product = HDB_RENOSMART_PRODUCT;
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="card-elevated rounded-[var(--radius-3xl)] p-10 md:p-14">
          <h1 className="text-3xl md:text-5xl font-black tracking-tight mb-4 text-center">
            Your HDB RenoSmart Planner is ready.
          </h1>
          ...
          {configError ? (
            ...
            <div className="flex flex-col items-center gap-4 mb-8">
              <a href={`/api/download/hdb-renosmart-planner?token=${encodeURIComponent(token!)}`} className="btn btn-primary btn-lg w-full sm:w-auto" download>
          ...
          <div className="rounded-2xl border border-[var(--color-border)] p-6 mb-8">
            ...
            <ul className="space-y-2 text-sm text-[var(--color-text-secondary)] leading-relaxed list-none pl-0">
              ...
              <li>• Start on the START HERE tab, then enter your flat type, floor area and scope on INPUTS.</li>
              <li>• BUDGET shows your personalised renovation cost with GST and contingency.</li>
              <li>• CASH FLOW breaks down payment milestones and your upfront cash requirement.</li>
              <li>• COMPLIANCE flags BTO toilet hacking, permits, and structural rules for your flat.</li>
            </ul>
          </div>

          <p className="text-xs text-[var(--color-text-tertiary)] leading-relaxed text-center mb-8">
            Estimates only — not financial advice. Need help? <Link href="/contact" className="underline hover:text-[var(--color-text-secondary)]">Contact us</Link> with your PayPal receipt email and we&apos;ll sort you out.
          </p>
