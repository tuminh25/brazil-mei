// src/app/thanks/hdb-tenant-pack/page.tsx
// Post-Purchase thank-you page for the HDB Tenant Protection & Cost Pack.
...
import { HDB_TENANT_PRODUCT } from "@/config/products";
...
export const metadata: Metadata = {
  title: "Your HDB Tenant Pack Is Ready — SGEventsHub",
  robots: { index: false, follow: false },
};

export default function HDBTenantPackThanksPage() {
  const product = HDB_TENANT_PRODUCT;
        ...
        <div className="card-elevated rounded-[var(--radius-3xl)] p-10 md:p-14">
          ...
          {configError ? (
            ...
            <div className="flex flex-col items-center gap-4 mb-8">
              <a href={`/api/download/hdb-tenant-pack?token=${encodeURIComponent(token!)}`} className="btn btn-primary btn-lg w-full sm:w-auto" download>
                Download the Pack ({product.priceLabel} · .xlsx)
              </a>
          ...
          <div className="rounded-2xl border border-[var(--color-border)] p-6 mb-8">
            ...
            <ul className="space-y-2 text-sm text-[var(--color-text-secondary)] leading-relaxed list-none pl-0">
              ...
              <li>• Start on the START HERE tab, then enter your numbers in the yellow cells on RENTAL CALCULATOR.</li>
              <li>• Print or use VIEWING CHECKLIST on your phone during viewings.</li>
              <li>• Complete PRE-SIGNING CHECKLIST before signing any agreement or paying any deposit.</li>
              <li>• Use MESSAGE TEMPLATES to clarify terms in writing — copy, adapt, send.</li>
              <li>• SOURCES & NOTES tab has official references and assumption disclosures.</li>
            </ul>
          </div>

          <p className="text-xs text-[var(--color-text-tertiary)] leading-relaxed text-center mb-8">
            This pack is a practical planning tool — not legal advice. Need help? <Link href="/contact" className="underline hover:text-[var(--color-text-secondary)]">Contact us</Link> with your PayPal receipt email and we&apos;ll sort you out.
          </p>
