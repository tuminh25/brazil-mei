// src/app/thanks/p1-phase-mapper/page.tsx
// Post-Purchase thank-you page for the P1 Phase & Priority Strategy Mapper.
...
import { P1_PHASE_MAPPER_PRODUCT } from "@/config/products";
...
export const metadata: Metadata = {
  title: "Your P1 Phase Mapper Is Ready — SGEventsHub",
  robots: { index: false, follow: false },
};

export default function P1PhaseMapperThanksPage() {
  const product = P1_PHASE_MAPPER_PRODUCT;
        ...
        <div className="card-elevated rounded-[var(--radius-3xl)] p-10 md:p-14">
          ...
          {configError ? (
            ...
            <div className="flex flex-col items-center gap-4 mb-8">
              <a href={`/api/download/p1-phase-mapper?token=${encodeURIComponent(token!)}`} className="btn btn-primary btn-lg w-full sm:w-auto" download>
                Download the Mapper ({product.priceLabel} · .xlsx)
              </a>
          ...
          <div className="rounded-2xl border border-[var(--color-border)] p-6 mb-8">
            ...
            <ul className="space-y-2 text-sm text-[var(--color-text-secondary)] leading-relaxed list-none pl-0">
              ...
              <li>• Start on the START HERE tab, then enter your child's profile (citizenship, sibling, alumni, volunteer).</li>
              <li>• PHASES shows all P1 registration phases (1 to 3) with eligibility, dates, and required documents.</li>
              <li>• SCHOOL SHORTLIST filters by actual road-distance tiers and priority.</li>
              <li>• TIMELINE builds a personalised action plan with deadlines and status tracking.</li>
            </ul>
          </div>

          <p className="text-xs text-[var(--color-text-tertiary)] leading-relaxed text-center mb-8">
            Planning tool only — not official MOE guidance. Need help? <Link href="/contact" className="underline hover:text-[var(--color-text-secondary)]">Contact us</Link> with your PayPal receipt email and we&apos;ll sort you out.
          </p>
