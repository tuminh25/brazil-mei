// src/app/api/download/transport-calculator/route.ts
...
export async function GET(request: Request) {
  ...
  const verification = verifyDownloadToken(
    ...
    TRANSPORT_CALCULATOR_PRODUCT.key
  );
  if (!verification.ok) {
  ...
  const masterPath = path.join(
    process.cwd(),
    "digital-products",
    "car-vs-mrt-grab-calculator",
    TRANSPORT_CALCULATOR_PRODUCT.masterAssetFile
  );
  ...
  return new Response(new Uint8Array(fileBuffer), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${TRANSPORT_CALCULATOR_PRODUCT.downloadFileName}"`,
