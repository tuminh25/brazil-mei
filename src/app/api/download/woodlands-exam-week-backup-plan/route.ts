// src/app/api/download/woodlands-exam-week-backup-plan/route.ts
// Signed-token gated PDF delivery. No DB, no auth, no external calls.
//
...
export async function GET(request: Request) {
  ...
  const verification = verifyDownloadToken(
    ...
    WOODLANDS_EXAM_BACKUP_PRODUCT.key
  );
  if (!verification.ok) {
  ...
  const masterPath = path.join(
    ...
    WOODLANDS_EXAM_BACKUP_PRODUCT.masterAssetFile
  );
  ...
  return new Response(new Uint8Array(fileBuffer), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${WOODLANDS_EXAM_BACKUP_PRODUCT.downloadFileName}"`,
