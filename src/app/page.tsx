...
export default async function HomePage() {
  ...
  // 1. HEADLINERS: Chỉ lấy đúng 3 bài Sắp diễn ra
    ...
    <main className={`${inter.className} min-h-screen bg-black text-white selection:bg-blue-500/30 overflow-x-hidden`}>
      ...
      <section className="relative w-full h-[85vh] flex flex-col items-center justify-center overflow-hidden border-b border-white/5">
        <div className="absolute inset-0">
          <img src="https://images.unsplash.com/photo-1543314223-999335f60647?q=80&w=1600&auto=format&fit=crop" className="w-full h-full object-cover opacity-40 scale-105" alt="Singapore Merlion" />
          <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/40 to-black"></div>
          <div className="absolute inset-0 bg-gradient-to-r from-blue-900/20 via-transparent to-purple-900/20"></div>
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
          <div className={`${mono.className} mb-8 text-blue-400 text-[10px] font-black uppercase animate-pulse`}>// Redefining Singapore Exploration</div>
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            ...
            <Link href="/attractions" className="bg-[#111] text-white border border-white/20 px-10 py-5 rounded-full font-black text-sm uppercase tracking-widest hover:bg-white hover:text-black transition-all">Top Attractions</Link>
          </div>
        </div>
      </section>

      {/* 2. UPCOMING HEADLINERS (HIỆN ĐÚNG 3 BÀI) */}
          ...
          {limitedEvents.map((event) => (
            <Link key={event.id} href={`/events/${event.slug}`} className="group relative bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] overflow-hidden hover:border-blue-500/50 transition-all">
               <div className="h-72 overflow-hidden relative">
                  ...
                  <div className="absolute top-4 right-4 bg-white text-black px-3 py-1 rounded-lg font-black text-[10px] uppercase tracking-widest">
                    {event.startDate ? event.startDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'TBA'}
                  </div>
               </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {latestGuides.map((post) => (
            <Link key={post.id} href={`/guides/${post.slug}`} className="group bg-[#111] border border-white/10 rounded-[2.5rem] overflow-hidden hover:border-green-500/50 transition-all duration-500">
               ...
               <div className="p-8">
                  <h2 className="text-2xl font-bold mb-4 line-clamp-2 text-white group-hover:text-green-400 transition-colors leading-tight">{post.title}</h2>
